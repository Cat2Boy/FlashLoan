...
describe("Token contract", function() {
    ...
    it("Deployment should assign the total supply of tokens to the owner", async function() {
        ...
        // 1、部署合约
        const flash = await ethers.getContractFactory("SimpleArbiV3",signer);
...
//     从KeepDao借钱  curve WETH->USDT   uniswapV3 USDT -> WETH

        // 4、构建 合约中  SwapData  struct

        const FORMAT = ['uint', 'uint256', 'uint256', 'address', 'address', 'address']

        let swapData1 = ethers.utils.AbiCoder.prototype.encode(
            FORMAT,
            [2,2,0,WETH,USDT,Curvepool]
        );
        let swapData2 = ethers.utils.AbiCoder.prototype.encode(
            FORMAT,
            [1,0,0,WETH,USDT,Curvepool]
        );
        
        let res = await instance.execute([swapData1,swapData2],ethers.utils.parseEther("5"));

        let profit = ethers.utils.formatEther(res);
